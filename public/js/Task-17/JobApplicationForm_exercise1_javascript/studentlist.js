async function liststudentdata(){
    const response = await fetch("/studentlistajax",{
        method:'GET'
    });
...
async function updatestudentdata(student){
    ...
    const response = await fetch(`/jobapplicationformupdateajax?student_id=${student_id}`,{
        method: 'GET',
        success: window.location.replace(window.location.protocol + "//" +
        window.location.host + `/jobapplicationformupdateajax?student_id=${student_id}`)
    });
}
...
function hidetable(){
    ...
    // xml.send();
    window.location.href = `${window.location.protocol}//${window.location.host}/jobapplicationformajax`;
}
